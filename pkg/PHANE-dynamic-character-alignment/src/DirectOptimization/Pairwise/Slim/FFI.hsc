...
import Layout.Compact.States (AlignmentStrategy(..), StateTransitionsCompact, TCMρ, getAlignmentStrategy, getFFI2D)
...
algn2d
  :: UnionContext
  -> MedianContext
  -> StateTransitionsCompact               -- ^ Structure defining the transition costs between character states
...
algn2d computeUnion computeMedians denseTCMs = directOptimization useC $ stateTransitionPairwiseDispersion denseTCMs
  where
    neverComputeOnlyGapped = 0
    costStruct = getFFI2D denseTCMs

    useC :: Vector SlimState -> Vector SlimState -> (AlignmentCost, SlimDynamicCharacter)
...
buildResult bufferLength alignedLength alignedBuffer = do
    ...
    let res = V.unsafeFromForeignPtr0 fPtr e :: Vector SlimState
    pure res
-}
...
{-# SPECIALISE coerceEnum :: Int       -> CSize #-}
{-# SPECIALISE coerceEnum :: Int       -> SlimState #-}
{-# SPECIALISE coerceEnum :: SlimState -> Int   #-}
{-# SPECIALISE coerceEnum :: Word      -> SlimState #-}
{-# SPECIALISE coerceEnum :: SlimState -> Word  #-}
